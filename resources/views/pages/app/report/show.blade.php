...
@if ($report->image && \Illuminate\Support\Facades\Storage::disk('public')->exists($report->image))
    <img src="{{ route('media', ['path' => $report->image]) }}" alt="" class="report-image mt-5">
@else
    <img src="{{ asset('assets/app/images/report-1.png') }}" alt="" class="report-image mt-5">
@endif
            ...
            <li class="timeline-item">
                <div class="timeline-item-content">
                    @if($status->image && \Illuminate\Support\Facades\Storage::disk('public')->exists($status->image))
                        <img src="{{ route('media', ['path' => $status->image]) }}" alt="status" class="img-fluid">
                    @endif
                    <span class="timeline-date">{{ \Carbon\Carbon::parse($report->created_at)->format('d M Y H:i') }}</span>
